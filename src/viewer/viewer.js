Potree.Viewer = class PotreeViewer extends THREE.EventDispatcher {
  ...
  addAnnotationFunc() {
    ...
    dummyMeasure.addEventListener("marker_dropped", markerDroppedEvent => {
      ...
      const annotationCancelButton = document.getElementById(
        "annotationCancel"
      );
      ...
      const annotationForm =
        annotationTitle.parentNode.parentNode.parentNode.parentNode.parentNode
          .parentNode;
      ...
      function handleFormSubmit() {
        ...
        function deleteAnnotation(annotationId) {
          if (localStorage.getItem("idToken")) {
            $.ajax({
              method: "DELETE",
              url: `/viewer/annotations/${modelId}?annotationId=${annotationId}`,
              headers: {
        ...
        // Edit.png is clear; removing the button causes delete button to break. Need to fix this
        let elTitle = $(
          `
					<span>
						` +
            escape(annotationTitle.value) +
            `
        ...
        elTitle.find("img[name=delete_annotation]").click(event => {
          ...
          annotations.children.forEach((anno, index) => {
            ...
            document.getElementById("myDiv" + index).innerHTML =
              anno.domElement[0].innerHTML;
            if (
              document.querySelectorAll(
                "#myDiv" + index + " div span span img"
              )[1]
            ) {
              if (
                document
                  .querySelectorAll("#myDiv" + index + " div span span img")[1]
                  .getAttribute("id") == event.target.id
              ) {
                annotations.remove(anno);
                anno.dispose();
                document.body.removeChild(
                  document.getElementById("myDiv" + index)
                );
        ...
        elTitle.toString = () => escape(annotationTitle.value);

        const annotation = $("#viewportChk").is(":checked")
          ? JSON.stringify({
              id: annotationId,
              modelId,
              position: markerDroppedEvent.measurement.points[0].position,
              content: {
                title: escape(annotationTitle.value),
                description: escape(annotationDescription.value) || null,
                cameraPosition: [
                ...
                cameraTarget: [
                  window.viewer.scene.view.getPivot().x,
                  window.viewer.scene.view.getPivot().y,
                  window.viewer.scene.view.getPivot().z
                ]
              }
            })
          : JSON.stringify({
        ...
        $("#add_annot").prop("disabled", false);

        if (localStorage.getItem("idToken")) {
          $.ajax({
            method: "POST",
            url: `/viewer/annotations`,
            headers: {
              Authorization: "Bearer " + localStorage.getItem("idToken")
            },
            data: annotation,
            ...
            error: function(error) {
              console.error("Anotation not saved: ", error);
            }
          });
        }
      }
      ...
      $(function() {
        ...
        $("#annotationTitle,#annotationDescription").bind("keypress", function(
          e
        ) {
