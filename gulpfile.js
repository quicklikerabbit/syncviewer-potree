...
gulp.task(
  "scripts",
  ["workers", "shaders", "icons_viewer", "examples_page"],
  function() {
    gulp
      .src(paths.potree)
...
gulp.task("watch", function() {
  ...
  let blacklist = ["resources/icons/index.html"];

  let watcher = watch(watchlist, cb => {
    {
      // abort if blacklisted
      let file = cb.path.replace(/\\/g, "/");
      let isOnBlacklist = blacklist.some(
        blacklisted => file.indexOf(blacklisted) >= 0
      );
      if (isOnBlacklist) {
        return;
      }
